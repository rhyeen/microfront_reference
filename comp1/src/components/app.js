...
class CompOneApp extends connect(store)(LitElement) {
  render() {
    return html`
    ${SharedStyles}
    <style>
  ...
  stateChanged(state) {
    this._container = state.comp1.containers[this.containerId]
    this._isLocked = state.comp1.locked
...
window.customElements.define('comp-one-app', CompOneApp);
